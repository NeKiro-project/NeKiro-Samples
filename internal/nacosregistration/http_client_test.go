...
func TestNewHTTPClientDisablesAmbientNetworkBehavior(t *testing.T) {
	config := validHTTPClientConfig()
...
func TestNewHTTPClientAuthenticatesPrivateCAAndOptionalClient(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			config := validHTTPClientConfig()
			config.APIOrigin = "https://nacos.internal:8848/nacos"
...
func TestNewHTTPClientRejectsUnsafeMaterialWithoutPathLeakage(t *testing.T) {
	directory := t.TempDir()
...
func validHTTPClientConfig() Config {
	config, err := Load(mapLookup(validEnvironment()), "RUNTIME_B", "runtime-b", "runtime-b-primary")
	if err != nil {
		panic(err)
	}
	return config
